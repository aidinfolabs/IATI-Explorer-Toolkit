<?php
  ...
  //The main script
   ...
   $prefix = EXIST_URI.EXIST_DB."?_query=";
   $query =  urlencode(($_GET['q'] ? str_replace("\\","",urldecode($_GET['q'])) : "//iati-activity"));
   $query.= "&_howmany=" .($_GET['howmany'] ? $_GET['howmany'] : "100");
  
  //Caching Routines
  $filename = "/tmp/explorer_json_". md5(str_replace("," , '_',str_replace('/','-',$query))).".json";
  if(file_exists($filename)) {
   ...
   $data = c_file_get_contents($prefix . $query,86400);
   ...
   if($xmlDoc->getElementsByTagName('iati-activity')->item(0)) {
	   $xslDoc = new DOMDocument();
   	   $xslDoc->load("iati-activities-xml-to-json-for-exhibit.xsl");
       ...
       $complete = 1;
	exError("We could not find IATI data for the country or option you selected"); 
...
// Functions
  function shutdown_timeout($complete) {
    ...
    if(!$complete) {
    	ob_end_clean();
	exError("The script could not fetch and process IATI data in the time available. It is possible that some of the process has completed and refreshing this page will allow you to view this data. If you repeatedly see this error, please let us know.");
    }
  }
...
//This function needs to be updated to work better
